...
// Build the country filter chip bar above the blog grid
function buildFilterBar(posts, activeCountry) {
    const section = document.querySelector('.blog-grid').closest('section');
    ...
    // Insert filter bar before the blog grid section's container
    ...
    if (activeCountry) {
        ...
        banner.className = 'blog-filter-banner';
        banner.id = 'filter-banner';
        banner.innerHTML = `
            <span>📍 Showing travel stories from <strong>${countryLabel}</strong></span>
            <a href="blog.html" class="filter-banner-clear">View All Stories ×</a>
        `;
...
function renderFilteredPosts(activeCountry) {
    ...
    if (filtered.length === 0) {
        ...
        empty.innerHTML = `
            ...
            <a href="blog.html" class="read-more" style="margin-top:1rem;display:inline-block;">View All Stories →</a>
        `;
